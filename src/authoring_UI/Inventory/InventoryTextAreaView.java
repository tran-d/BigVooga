...
public class InventoryTextAreaView extends DisplayableTextAreaView {
	...
	private static final double INVENTORY_PROMPT_WIDTH = 250;
	private static final double INVENTORY_PROMPT_HEIGHT = 250;
	...
	protected InventoryTextAreaView(Runnable save, Supplier <Color> bgColor) {
		...
		this.getChildren().addAll(inventoryPreview, makeToolPanel());
	...
	protected void addPanel() {
		paneList.add(currentPaneIndex+1, createPane(INVENTORY_PROMPT_WIDTH, INVENTORY_PROMPT_HEIGHT));
	...
	protected void addTextArea() {
		...
		taList.add(ta);
		Pane k = (Pane) inventoryPreview.getChildren().get(0);
	...
	protected void setCurrentPane() {
		...
		inventoryPreview.getChildren().add(paneList.get(currentPaneIndex));
	}
